...
- [scripts/fetch_data.py](scripts/fetch_data.py) henter data fra gratis API-er (Frankfurter/ECB, Norges Bank, BIS, OECD, FRED, CFTC, World Bank og sentralbankenes kurvedata – ingen nøkler) og skriver `data/dashboard.json` og `data/history.json`. Rentebanen for USD, AUD og CAD leses ut av futures på styringsrenten (fed funds via Yahoo, ASX 30-dagers cash rate, CORRA fra Montréal-børsen), som gir prising per måned og per rentemøte; statskurven brukes for punktene og for banen utover siste kontrakt. GBP bruker BoE OIS, NZD swapkurven fra RBNZ B2 (lastet ned med Playwright i [scripts/fetch_rbnz.py](scripts/fetch_rbnz.py), siden rbnz.govt.nz ligger bak Cloudflare), og øvrige statspapirkurver. AUD og CHF har statskurver som publiseres månedlig (RBA F17, SNB `rendeiduebd`) og forskyves daglig med markedsrentene.
