...
- [ ] **Ulike instrumenter / terminpremie.**
  ...
  *Løsning:*
  1. Anker banen på markedets frontrente, ikke styringsrenten: `path[0] = r_3m − s`, der `s` er rullerende 250-dagers median av `r_3m − styringsrente`. Nivået flytter seg bare når markedet flytter seg, og «vedtak levert»-hoppet forsvinner (løser også reprisingspunktet under).
  2. Bytt til møtebaserte instrumenter der de finnes gratis: USD = fed funds-futures per måned via yfinance (`ZQF27.CBT`-format, verifiser ticker), AUD = ASX RBA Rate Tracker (HTML, implisitt yield per kontrakt), CAD = TMX CORRA-futures (delayed HTML), GBP = BoE OIS (har vi). EUR/SEK/NOK/JPY: behold statspapirer + spread-korreksjon, merk «statskurve» i visningen.
