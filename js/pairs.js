...
import { nb1, nb2, pct, pct1, rate, signed, moves, cls } from "./format.js";
...
export function renderPairs(dashboard, history) {
  ...
  const describeLeg = (c) => {
    ...
    if (i.m3 != null) bits.push(`en kurs som har gått <b>${pct(i.m3)}</b> mot G10-kurven siste 3 mnd`);
  ...
  const render = () => {
    ...
    const table = rows.map((r) => `<tr>
        <td><b>${r.L.flag} ${r.L.currency}</b> / <b>${r.S.flag} ${r.S.currency}</b></td>
    ...
    document.getElementById("pairResult").innerHTML = `
      ...
      <p class="lead">Ingen valuta er nøytral motpart: den du ${isLong ? "selger" : "kjøper"} har egen rente, egen risikoprofil og egen energieksponering.
        Forslagene under viser hva du får med på kjøpet.</p>
      ...
      <details class="more"><summary>Slik regnes det</summary><p class="note">Renteforskjell = 1-års rente i long-beinet minus short-beinet.
        ...
        «Lik risikoprofil?» = om begge valutaene svinger likt med risikoappetitten (AUD/JPY) siste 90 dager, målt mot G10-kurven. Svingninger = realisert volatilitet
